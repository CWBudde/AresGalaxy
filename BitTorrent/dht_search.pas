...
uses
  Classes, Windows, Math, SysUtils, SynSock, Classes2, dht_int160, dht_consts;
...
uses
 helper_datetime, helper_strings, helper_ipfunc, thread_bittorrent,
 dht_socket, utility_ares, DHT_SearchManager;
...
begin
	if m_possible.count=0 then
  begin
    if m_type<>NODE then
    begin
      if ((m_created+SEC(10)<time_now)) then
      begin
	      expire;
	      exit;
      end;
  ...
  if m_lastResponse+SEC(3)>time_now then
    exit;
  ...
  donecount := 0;
  count := 4;
  while ((m_possible.count>0) and (donecount<count)) do
  begin
	  bucket := m_possible[0];
    m_possible.delete(0);

    if has_contacts_withid(m_tried,@bucket.ID) then
      continue;

		// Move to tried
	  m_tried.add(bucket);

    if (m_type=dht_consts.FINDSOURCE) and (bucket.ID[0] xor m_target[0] < MDHT_SEARCH_TOLERANCE) then
    begin
...
begin
  ...
  begin
    ...
    if CU_INT160_compare(id,@c.id) then
    begin
      Result := True;
      exit;
    end;
...
begin
  ...
  for h := 0 to m_tried.count-1 do
  begin
		result := m_tried[h];

		if ((result.ipC=IP) and (result.portW=Port)) then
      exit;
  end;
...
begin
  ...
  begin
	...
	if m_type=dht_consts.NODE then
  begin
		inc(m_answers);
		m_possible.clear;
		results.clear;
  ...
  begin
    ...
    if c.ID[0] xor m_target[0]>from.ID[0] xor m_target[0] then
    ...
    begin      // add him only if he's better then the worst one
      thread_bittorrent.mdht_sortCloserContacts(m_best,@m_target);
      worstContact := m_best[m_best.count-1];

      if c.ID[0] xor m_target[0] < worstContact.ID[0] xor m_target[0] then
      begin
    ...
    begin
      ...
      if (m_type=dht_consts.FINDSOURCE) and (c.ID[0] xor m_target[0] < MDHT_SEARCH_TOLERANCE) then
      begin
...
begin
  ...
  for i := 0 to m_possible.count-1 do
  begin
    bucket := m_possible[i];
    m_inuse.add(bucket);
  end;
...
begin
	...
	case m_type of
    dht_consts.NODE,
		dht_consts.NODECOMPLETE:
      baseTime := dht_consts.MDHT_SEARCHNODE_LIFETIME;
		dht_consts.FINDSOURCE:
      baseTime := dht_consts.MDHT_SEARCHFINDSOURCE_LIFETIME
    else
      baseTime := dht_consts.MDHT_SEARCH_LIFETIME;
	end;
...
begin
  ...
  //d1:ad2:id20:abcdefghij01234567896: Target20:mnopqrstuvwxyz123456e1:q9:find_node1: T2:aa1:y1:qe
  outstr := 'd1:ad2:id20:' + me160 +  '9:info_hash20:' + target +
    'e'+ '1:q9:get_peers'+ '1: T2:' +
    int_2_word_string(mdht_currentOutpacketIndex)+ '1:y1:qe';
