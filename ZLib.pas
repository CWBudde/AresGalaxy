...
function ZDecompress(const inBuffer: Pointer; inSize: Integer;
  out outBuffer: Pointer; out outSize: Integer; outEstimate: Integer = 0): Boolean;

  function ZDecompress2(const inBuffer: Pointer; inSize: Integer;
  out outBuffer: Pointer; out outSize: Integer; outEstimate: Integer = 0): Boolean;
{** string routines *********************************************************}
...
function ZDecompressStr2(const s: string): string;
...
function ZDecompress2(const inBuffer: Pointer; inSize: Integer;
  out outBuffer: Pointer; out outSize: Integer; outEstimate: Integer): Boolean;
var
  zstream: TZStreamRec;
  delta,res: Integer;
begin
  ...
  try
   ...
   exit;
    ...
    if InflateInit2(zstream,-MAX_WBITS)<0 then begin
      try
      FreeMem(outBuffer);
      except
      end;
      ...
      while true do begin
       ...
       if res=Z_STREAM_END then break;
       ...
       if inflateEnd(zstream)<0 then begin
        try
        FreeMem(outBuffer);
        except
        end;
...
begin
  ...
  try
   ...
   exit;
    ...
    if InflateInit(zstream)<0 then begin
      try
      FreeMem(outBuffer);
      except
      end;
...
var
  ...
  outBytes: Integer;
begin
  if ZDecompress(pointer(s), Length(s), outBuf, outBytes) then begin
   SetLength(s, outBytes);
   MoveI32(pointer(outBuf)^, pointer(s)^, outBytes);
   FreeMem(outBuf);
  end;
...
var
  ...
  size: Integer;
begin
  if ZDecompress2(PChar(s), Length(s), buffer, size) then begin
   SetLength(result, size);
   Move(buffer^, pointer(result)^, size);
   FreeMem(buffer);
  end else Result := '';
...
var
  ...
  size: Integer;
begin
  if ZDecompress(PChar(s), Length(s), buffer, size) then begin
   SetLength(result, size);
   Move(buffer^, pointer(result)^, size);
   FreeMem(buffer);
  end else Result := '';
